...
function ($scope, $stateParams, $http, $state,$ionicLoading,Base_URL,$window,$cordovaInAppBrowser,$rootScope,$ionicPlatform,$ionicPopup,$httpParamSerializer){
//var paymentstatus = JSON.stringify(payment.response.state);
  //var paymentst = paymentstatus.replace(/\,/g,"");
...
//      } 
//   };
       var wallet = $httpParamSerializer({
           uid:userid,
           total:$scope.data.firstname
...
//alert($scope.tdata);
  ...
  $http.defaults.headers.post['Content-Type'] = 'application/x-www-form-urlencoded; charset=UTF-8';
   $http.post(Base_URL+'api/users/walletcc',wallet).success(function(response){
  // alert("235668");
    console.log(response);
   ...
   //alert("my"+JSON.stringify(event));
   $scope.param=event.url.split("/");
  $scope.status=$scope.param[$scope.param.length-1]
          $cordovaInAppBrowser.close();
         //alert($scope.status)
         if($scope.status == 1){
              $rootScope.loyalty_points1=parseFloat($rootScope.loyalty_points1)+parseFloat($scope.price);
              $scope.price="";
              delete $scope.price
              alert("true");
              var myPopup = $ionicPopup.show({
                    template:"Amount successfully added in Dropin wallet",
                    scope: $scope,
                    cssClass: 'value_sec',
                    buttons: [
             ...
             var myPopup = $ionicPopup.show({
                    template:"Some internal error, please try again",
                    scope: $scope,
                    cssClass: 'value_sec',
                    buttons: [
                      { text: '<span class="oky">Okay</span>',
        ...
        // alert("bhumika"+  $stateParams.id);
      //   $state.go("menu.wallet", {id: $scope.status});
        //  $state.go("");
        ...
        var datemq = $ionicPopup.show({
                            // title:'Oops',
                         subTitle: 'Something going wrong',
                             cssClass: 'value_sec',
                             scope: $scope,
                             buttons: [
                             { text: '<span class="oky">Okay</span>',
...
//console.log($window.localStorage.getItem('wallet_payment'));
   ...
   $http.post(Base_URL+'api/shop/walletpayment',$scope.all).success(function(response){
    ...
    console.log(response);
     if(response.error == "0")
    {
        $rootScope.loyalty_points1=parseFloat($rootScope.loyalty_points1)-parseFloat($rootScope.last_totalpick);
  //  alert($rootScope.loyalty_points1);
      console.log($rootScope.loyalty_points1)
...
//       $window.localStorage.setItem('orderwallet',JSON.stringify(response.data));
     if(response.error == "0")
    {
       $rootScope.loyalty_points1=parseFloat($rootScope.loyalty_points1)-parseFloat($rootScope.last_total);
       console.log($rootScope.loyalty_points1)
//    $window.localStorage.setItem('orderwallet',JSON.stringify(response.data));
//    console.log($window.localStorage.getItem('orderwallet'));
    $state.go("menu.confirmation");
...
// You can include any angular dependencies as parameters for this function
// TIP: Access Route Parameters for your page via $stateParams.parameterName
            function ($scope, $stateParams, $state, $ionicSlideBoxDelegate,$window,$ionicPlatform,$ionicPopup,$location,$ionicHistory,$rootScope) {
...
//                                    }
    ...
    if (true) {
        ...
        .controller('step1Ctrl',function ($ionicPlatform, $scope, $http, $window,$cordovaGeolocation, $state,$ionicHistory,$ionicPopup,$rootScope,$ionicLoading,Base_URL) {
            $scope.data = {};
...
// You can include any angular dependencies as parameters for this function
// TIP: Access Route Parameters for your page via $stateParams.parameterName
                function ($scope, $stateParams, $ionicModal, $state, $rootScope, $cordovaGeolocation, $window, $http,$ionicPopup,$ionicLoading,Base_URL,$ionicPlatform,$location,$ionicHistory) {
...
//    $rootScope.seller_group_id = 1;
 ...
 $ionicPlatform.registerBackButtonAction(function(event) {
  ...
  $scope.isGroupShown3= function(group3) {
    ...
    $scope.uid=(JSON.parse($window.localStorage.getItem('User_Data'))).user_id;
    $scope.id=(JSON.parse($window.localStorage.getItem('rest_id')));
        console.log($scope.uid);
    $scope.rdata={
        Review:{
            resid:JSON.parse($window.localStorage.getItem('rest_id')),
            uid:$scope.uid
        }
    };
    console.log($scope.rdata);
      ...
      $scope.rdata = {
        ...
        $scope.rdata = {
          ...
          // console.log($stateParams.restid);
            ...
            $rootScope.dis = $stateParams.dis;
            $scope.userid=$window.localStorage.getItem("user_id");
                $rootScope.riidd=JSON.parse( $window.localStorage.getItem('rest_id'));
            $scope.rdata = {
                Restaurant: {
                    id: $rootScope.riidd
...
$ionicLoading.show();
            $http.post(Base_URL+'api/restaurants/restaurantbyid', $scope.rdata)
                    .success(function (response) {
                       ...
                       $rootScope.cancel_time_for_dine=response.data.cancel_time;
                    });
...
//    }
                    ...
                    $scope.scrollEvent = function () {
                     ...
                     $scope.timedine=function(time_pi){
        
//    if($rootScope.date_pickup == '1'){
//        alert("select date")
       $rootScope.dine_time=time_pi;
//    }else{
//     $rootScope.rest_closed=0;
       // console.log(time_pi);

}
                    
                    
 $scope.dineorder=function(dine_req){
                $rootScope.rest_dine=dine_req;
                if($rootScope.rest_dine == undefined){
                     $rootScope.rest_dine="null"
                     console.log($rootScope.rest_dine)
                 }
                console.log($rootScope.rest_dine);
                if($rootScope.dinedate == undefined){
                    var myPopup = $ionicPopup.show({
                    ...
                    if($rootScope.dine_time == undefined){
                         var myPopup = $ionicPopup.show({
             ...
             $scope.pickpdata = {
                Restaurant: {
                    res_id: $scope.restid,
                    selected_date:$scope.data.date,
                    selected_time:$scope.data.time,
                    delivery_order:1
                    }
                };
                console.log($scope.pickpdata);
                $ionicLoading.show();
                $http.post(Base_URL+'api/restaurants/restaurantavailable.json', $scope.pickpdata)
                .success(function (response) {
                $ionicLoading.hide();
                console.log(response);
                       ...
                       //$window.localStorage.setItem('tablepayment', JSON.stringify($rootScope.tb));
                        ...
                        confirmPopup.then(function (res) {
                            if (res) {
                               ...
                               $window.localStorage.setItem('timecard', $scope.data.time);
                               console.log(  $window.localStorage.getItem('timecard'));
                              $window.localStorage.setItem('carddate', $scope.data.date);
            ...
            $http.post(Base_URL+'api/products/getsingleproduct', $scope.adata).success(function(rest) {
                ...
                $ionicLoading.hide();
                   ...
                   if (rest.error == 0) {
                    ...
                    $rootScope.assoc_products = rest.list.Product.AssoPro;
                    console.log($scope.assoc_products);
                   // console.log($scope.assoc_products)
...
//                 
                    ...
                    $scope.uniqueID = JSON.parse($window.localStorage.getItem('User_Data')).uniqueID;
                       $scope.displaycart = function (items) {
                           if(items == undefined){
                             var my = $ionicPopup.show({
                        ...
                        $http.post(Base_URL+'api/shop/displaycart', $scope.udata).success(function (response) {
                            ...
                            console.log(response);
                         $rootScope.cartitem = response.data[1];
                         $rootScope.pickupcartitem = response.filtered_array;
                          /*angular.forEach($rootScope.pickupcartitem ,function(key,value){
                              console.log(key);
                              console.log(value);
                              
                          })*/
                            $rootScope.surchargepickup = response.surcharge.pickup_surcharge;
                           // $rootScope.item = response.data[0];
                            $rootScope.qnt = response.data[0];
     ...
     //$window.location.reload(); //close the popup after 3 seconds for some reason
            ...
            $http.post(Base_URL+'api/products/getsingleproduct', $scope.adata).success(function(rest) {
                ...
                if (rest.error == 0) {
                     ...
                     $scope.baseprice=$scope.add_product.price;
                      $rootScope.totalprice=$scope.baseprice;
                      $rootScope.dine_ass=rest.list.Product.AssoPro;
                      console.log($rootScope.dine_ass);
                    //$rootScope.test_id =$scope.add_product.id;
                    $scope.data.parent_id = $scope.add_product.id;
                  console.log($scope.add_product);
//                    var asso = [];
  ...
  console.log($rootScope.totalprice);
    ...
    $scope.ldata = $window.localStorage.getItem('restdata');
            ...
            if($window.localStorage.getItem('deliverymethod') != $window.localStorage.getItem('ordermethodpick')){
                    ...
                    $scope.userid = JSON.parse($window.localStorage.getItem('User_Data')).user_id;
                    $scope.uniqueID = JSON.parse($window.localStorage.getItem('User_Data')).uniqueID;
                    $scope.displaydine = function (items) {
                       // alert(items);
                       if(items == undefined){
                        ...
                        $http.post(Base_URL+'api/shop/displaycart', $scope.udata).success(function (response) {
                           ...
                           // console.log($rootScope.dinetotal);
                           //  console.log($rootScope.dinetotal.total);
                             $rootScope.surcharge = response.surcharge.delivery_surcharges;
                              console.log($rootScope.surcharge);
                            $rootScope.cartitem = response.data[1];
                            $rootScope.dine_cartitem = response.filtered_array;
                            console.log($rootScope.cartitem);
                            $rootScope.dinetotal = response.data[0];
                            console.log($rootScope.dinetotal);
                             console.log($rootScope.dinetotal.total);
...
$scope.piskmenu=function(pdfid){
    ...
    //alert(pdfid)
                    ...
                    $http.post(Base_URL+'api/products/getsingleproduct', $scope.addata) .success(function (response) {
                             ...
                             $rootScope.assoc_products= response.list.Product.AssoPro;
                    console.log($scope.assoc_products);
                  ...
                  //   alert(pid);
                    ...
                    $http.post(Base_URL+'api/products/getsingleproduct', $scope.addata) .success(function (response) {
                            ...
                            $rootScope.pdetails=response.list.Product;
                             $rootScope.dine_ass=response.list.Product.AssoPro;
                      console.log($rootScope.dine_ass);
...
// TIP: Access Route Parameters for your page via $stateParams.parameterName
                function ($scope, $stateParams, $rootScope, $window, $http,$ionicLoading,Base_URL,$state,$ionicPopup) {
                    ...
                    ///////////////////remove/////////////////////////////
                                             $scope.remove_dish11 = function($cart_id, product_id) {
                             alert(product_id);
                             alert($cart_id);
                             console.log(product_id);
                             
                             console.log($cart_id+"uhdfvd");
				 $scope.adata = {
                Product: {
                    id: product_id
                }};
				$http.post(Base_URL+'api/products/getsingleproduct', $scope.adata).success(function(rest) {
                                    console.log(rest);
					console.log(rest.list.Product.AssoPro);
                                        console.log(rest.list.Product.id)
					if(rest.list.Product.AssoPro != null){
						console.log(rest.list.Product);
                        ...
                        angular.forEach($rootScope.cartitem, function(value1,index1){
							...
							angular.forEach(rest.list.Product.AssoPro,function(value2,index2){
								console.log(value2.id);
                                                                console.log("bhumika");
		//alert(value1.Cart.product_id+"//"+value2.Product.id)
	if(value1.Cart.product_id== value2.id){
            console.log("my")
		$scope.cartdata = {
                    Cart: {
                        id: value1.Cart.id
                        ...
                        $http.post(Base_URL+'api/shop/displaycart', $scope.udata).success(function(response) {
                           ...
                           // $rootScope.item = response.data[0];
                            ...
                            $rootScope.totalpick = response.data[0].total;
                                    console.log($rootScope.totalpick);
                                    $rootScope.total=parseFloat($rootScope.surchargepickup)+parseFloat($rootScope.totalpick);
                                    console.log(typeof $rootScope.total);
                                    $window.localStorage.setItem('subtotal1', JSON.stringify($rootScope.total));
                    ...
                    if (rest.error == 0) {
                        ...
                        $http.post(Base_URL+'api/shop/displaycart', $scope.udata).success(function(response) {
                           ...
                           // $rootScope.item = response.data[0];
                            $rootScope.qnt = response.data[0];
                            $rootScope.totalpick = response.data[0].total;
                                    console.log(typeof $rootScope.totalpick);
                                    $rootScope.total=parseFloat($rootScope.surchargepickup)+parseFloat($rootScope.totalpick);;
                                    $window.localStorage.setItem('subtotal1', JSON.stringify($rootScope.total));
                                    $rootScope.numbitemsd = response.data[0].order_item_count;
                                if($rootScope.product_details.length == 0){
...
$scope.userid = JSON.parse($window.localStorage.getItem('User_Data')).user_id;
                    $scope.removeitem = function ($cart_id,product_id)
                    {
                     //   alert("remove");
                        //var carts = [];
                        //$rootScope.cart='';
...
//			$scope.taxes();
                    ...
                    $scope.inc = function ($cart_id, quantity)
                    {
                       // alert(quantity);
...
$ionicLoading.hide();
                    ...
                    $scope.dec = function ($cart_id,dec_quantity)
                    {
                      //  alert(dec_quantity);
                        if(dec_quantity != 1){
                        $scope.cartdata = {
                        ...
                        var myPopup = $ionicPopup.show({
                                        ...
                                        title: "Oops!",
                                        template:"Item quantity should not be less than 1",
                                        cssClass: 'value_sec',
                                        scope: $scope,
                                        buttons: [
                                          { text: '<span class="oky">Okay</span>',
...
// TIP: Access Route Parameters for your page via $stateParams.parameterName
                function ($scope, $stateParams, $window, $http, $rootScope,$ionicLoading,Base_URL,$state,$ionicPopup) {
                    ...
                    $rootScope.userid = JSON.parse(localStorage.getItem('user_id'));
                     ...
                     $scope.removedine = function ($cart_id,product_id)
                    {
                     //   alert("remove");
                        //var carts = [];
                        //$rootScope.cart='';
...
//			$scope.taxes();
                   ...
                   //**************** remove **********//
                         $scope.remove_dish67657 = function($cart_id, product_id) {
                             //alert("dsyuhfgduy");
                         //    alert(product_id);
                           //  alert($cart_id);
				 $scope.adata = {
...
$ionicLoading.hide();
                    ...
                    $scope.dec = function ($cart_id,quantity)
                    {
                        if(quantity != 1){
                        $scope.cartdata = {
                            Cart: {
...
// You can include any angular dependencies as parameters for this function
// TIP: Access Route Parameters for your page via $stateParams.parameterName
            function ($scope,$window,$state,$rootScope){
                $scope.data ={};
...
$scope.data.firstname= $rootScope.User_data.name;
$scope.data.phone= $rootScope.User_data.phone;
...
//                    
        ...
        .controller('confirmationCtrl', 
            function ($rootScope,$scope,$window,$state,Base_URL,$http,$ionicHistory,$ionicSideMenuDelegate,$ionicLoading,$cordovaGeolocation,$ionicPopup) {
                        ...
                        $http.post(Base_URL+'api/shop/displaycart', $scope.udata).success(function(response) {
                            ...
                            if (response.error == 0) {
                               ...
                               console.log(response.data[1][0].Cart.resid);
                                if(response.data[1][0].Cart.resid !=JSON.parse($window.localStorage.getItem('rest_id'))){
...
// You can include any angular dependencies as parameters for this function
// TIP: Access Route Parameters for your page via $stateParams.parameterName
    function ($scope, $stateParams,$rootScope,$state,$window,$http,$ionicLoading,$cordovaGeolocation,Base_URL,$filter,$ionicPopup,$ionicHistory,$timeout,$httpParamSerializer)
    {
//                 $ionicLoading.show({ 
  ...
  $scope.reservepending=function(){
      ...
      var view= $httpParamSerializer({
           ...
           // $ionicLoading.show();
            console.log(view);
            $http.defaults.headers.post['Content-Type'] = 'application/x-www-form-urlencoded; charset=UTF-8';  
            $http.post(Base_URL+"api/shop/lasttable", view).then(function (response){
            
            console.log(response.data);
           // console.log(response.data.data[0].Restaurant);
           // console.log(response.data.data[0].TableReservation);
            
             //console.log($scope.sdsef);
            if(response.data.error == "0"){
              $scope.sdsef = $filter('date')(new Date(), 'dd-MM-yyyy');
            $scope.order_date=$filter('date')(new Date(response.data.data[0].TableReservation.d_day), 'dd-MM-yyyy');
                if($scope.sdsef > $scope.order_date)
               {
                   console.log("bhumika")
                   $rootScope.dispay_none=0;
               }else{
                   $rootScope.dispay_none=1
               }
                
               $rootScope.rst_name=response.data.data[0].Restaurant.name;
               $rootScope.rst_day=response.data.data[0].TableReservation.d_day;
                $rootScope.D_status=response.data.data[0].TableReservation;
                 $rootScope.rst_Time=response.data.data[0].TableReservation.d_time;
                  $rootScope.rst_People=response.data.data[0].TableReservation.no_of_people;
         }else{
             $rootScope.dispay_none=0;
           // $ionicLoading.hide();  
...
//                        
//                   if (new Date($rootScope.table[i].TableReservation.d_day) == $scope.sdsef){
...
// TIP: Access Route Parameters for ur page via $stateParams.parameterName
            function ($scope, $stateParams,$window,$http,$rootScope,Base_URL,$ionicLoading,$cordovaCamera,$ionicPopup,$state) {
                
             $scope.data ={};
             console.log(JSON.parse($window.localStorage.getItem("user_id")));
             console.log( $scope.name);
             $scope.id= JSON.parse($window.localStorage.getItem("user_id"));
$scope.data.pro_name= $rootScope.User_data.name;
$scope.data.pro_phone= $rootScope.User_data.phone;
$rootScope.profilepic= $rootScope.User_data.image
...
// You can include any angular dependencies as parameters for this function
// TIP: Access Route Parameters for your page via $stateParams.parameterName
            function ($scope, $stateParams,$rootScope,$http,Base_URL,$window,$ionicLoading,$cordovaInAppBrowser,$ionicPopup,$state,$httpParamSerializer) {
                  var day1 = [];
                    $scope.data = {};
                    var date = new Date();
                    $rootScope.n = date.getDate();
                    ...
                    $http.post(Base_URL+'api/shop/time', $scope.day_id).success(function (response)
                    {
                     ...
                     $scope.kj=function(card,indexValue){
                         delete $rootScope.rest_time;
                        //alert(card);
                        //alert(date);
                       $rootScope.pichdate = card;
                        if(typeof $rootScope.date !='undefined'){
                        $scope.start_date = $rootScope.date[0]
                        var alignFillDate = new Date(card);
                         $rootScope.date_pickup=0;
                        //alert($rootScope.date_pickup);
                        var pickUpDate = new Date($scope.start_date);
                        var Xmas95 = new Date(card);
var weekday = Xmas95.getDay();

$rootScope.day_pick=weekday;
//console.log(weekday);
//console.log(weekday);
//var n = alignFillDate.toUTCString();
//console.log(n);
//var res = n.split(",");
//console.log(res[0]);
                        ...
                        console.log(pickUpDate);
                        console.log(alignFillDate);
                        console.log($rootScope.open_time);
                        console.log($rootScope.close_time);
                        if (angular.equals(pickUpDate, alignFillDate)) {
                        //$rootScope.decrement_flag = 1;
                        console.log('same date')
			$scope.day_id={
    ...
    $scope.timepick=function(time_pi){
        
//    if($rootScope.date_pickup == '1'){
//        alert("select date")
       $rootScope.rest_time=time_pi;
//    }else{
//     $rootScope.rest_closed=0;
       // console.log(time_pi);
...
$scope.restid=JSON.parse ($window.localStorage.getItem("rest_id"));
 ...
 //console.log($scope.restid);
   $scope.rpdata = {
              ...
              //  console.log($rootScope.reservationdetail);
                $rootScope.webdetp = response.data.website;
               $rootScope.cancel_time_for_pickup = response.data.cancel_time;
                });
                $scope.webres=function(){ 
                var options = {
                location: 'yes',
       ...
       // alert(event);
            ...
            $scope.pickuporder=function(request){
                // console.log(request);
                 $rootScope.req_pick=request
                 if($rootScope.req_pick == undefined){
                     $rootScope.req_pick="null"
                     console.log($rootScope.req_pick)
                 }
                //alert( $rootScope.rest_time);
                if($rootScope.pichdate == undefined){
                    var myPopup = $ionicPopup.show({
                    ...
                    if($rootScope.rest_time == undefined){
                         var myPopup = $ionicPopup.show({
...
//                                console.log($window.localStorage.getItem('card'));
    ...
    .controller('contactCtrl',  // The following is the constructor function for this page's controller. See https://docs.angularjs.org/guide/controller
// You can include any angular dependencies as parameters for this function
// TIP: Access Route Parameters for your page via $stateParams.parameterName
            function ($scope, $stateParams,$rootScope,$http,Base_URL,$window,$ionicLoading,$cordovaInAppBrowser,$ionicPopup,$state,$httpParamSerializer) {
                $scope.data={};
   ...
   if(response.msg == "Success"){
    ...
    console.log($scope.data.reason)
     ...
     var contact = $httpParamSerializer({
            ...
            $ionicLoading.show();
            $http.defaults.headers.post['Content-Type'] = 'application/x-www-form-urlencoded; charset=UTF-8';
            $http.post(Base_URL+'api/testapi/contactus', contact)
                 .success(function (response)
                     {
                         if(response.error == 0){
                             $ionicLoading.hide();
